...
Component({
  ...
  methods: {
        ...
        function () {
          this.downloadImages(this.properties.palette).then(async palette => {
            ...
            if (needScale) {
			  ...
			  console.log('pixelRatio',getApp().systemInfo.pixelRatio)
			  console.log('this.photoContext.width',this.canvasWidthInPx)
			  console.log('this.photoContext.height',this.canvasWidthInPx)
            }
			
			// new Pen(this.photoContext, palette).paint(callbackInfo => {
			// 	console.log('callbackInfo2',callbackInfo)
			//   this.saveImgToLocal();
			// });
			
            new Pen(this.photoContext, palette).paint(() => {
				// console.log('callbackInfo2')
              this.saveImgToLocal();
            });
            setStringPrototype(this.screenK, this.properties.scaleRatio);
          });
    ...
    saveImgToLocal() {
      ...
      const that = this;
      setTimeout(() => {
		  console.log('savethat.canvasNode',that.canvasNode)
        wx.canvasToTempFilePath(
          {
            ...
            success: function (res) {
				console.log('hsddddd',res.tempFilePath ,that.canvasWidthInPx,that.canvasHeightInPx)  
